<template>
    <form id="updateCompaignForm" method="post" action="#">
        <b-field label="Name"
        ...
        <div class="tags">
            <span v-for="(file, index) in files_to_keep"
                  class="tag is-primary" >
                {{getFileName(file)}}
                <button class="delete is-small"
                        type="button"
                        @click="deleteUploadedDropFile(index)">
                </button>
            </span>
...
<script>
    export default {
        name: "UpdateCompaign",
        props: ['name' , 'from_update' , 'to_update' , 'daily_budget' , 'id', 'files_to_keep'],
        data() {
            return {
                from: new Date(this.from_update),
                to: new Date(this.to_update),
                dropFiles: [],
                nameValidatorType: '',
        ...
        mounted() {
          this.from = new Date(this.from_update);
          this.to = new Date(this.to_update);
          this.files_to_keep = JSON.parse(this.files_to_keep);
          console.log('mounted here : ' , this.from , this.to , this.files_to_keep);
        },
        methods: {
            deleteDropFile(index) {
                this.dropFiles.splice(index, 1)
            },
            deleteUploadedDropFile(index) {
                this.files_to_keep.splice(index, 1)
            },
            clearFrom () {
                this.from = null
            },
            ...
            getFileName(file) {
              return file.substring(file.indexOf('pictures/') + 9 , file.length);
            },
            submitCompaign() {
                this.loading = true
                ...
                axios({
                    method: 'POST',
                    url: `/compaigns/${this.id}/update?name=${this.name}&from=${from_date}&to=${to_date}
                          &daily_budget=${this.daily_budget}&files_to_keep=${this.files_to_keep}`,
                    data: data
                }).then(({data}) => {
                    this.loading = false;
